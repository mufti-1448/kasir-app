...
@section('content')
    ...
    <select id="barang_id" class="form-control mb-1">
        ...
        @foreach ($barangs as $barang)
            <option value="{{ $barang->id }}" data-harga="{{ $barang->harga_jual }}" data-nama="{{ $barang->nama }}" data-stok="{{ $barang->stok }}">
                {{ $barang->kode }} - {{ $barang->nama }} (Stok: {{ $barang->stok }}) - Rp {{ number_format($barang->harga_jual,0,',','.') }}
            </option>
        @endforeach
    </select>
    <small class="text-muted" id="stok-info" style="min-height: 20px;"></small>
</div>
<div class="col-md-3">
