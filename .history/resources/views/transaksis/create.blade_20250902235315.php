                                    ...
                                    <select id="barang_id" class="form-control mb-1">
                                        ...
                                        @foreach ($barangs as $barang)
                                            <option value="{{ $barang->id }}" data-harga="{{ $barang->harga_jual }}"
                                                ...
                                                data-kode="{{ $barang->kode }}"> <!-- Tambahkan ini -->
...
const displayKode = kode ? kode + ' - ' : ''; // hanya tampilkan "-" kalau kode ada

newRow.innerHTML = `
<td>${displayKode}${nama}</td>
...
`;
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Hapus input hidden lama jika ada (opsional)
                form.querySelectorAll('input[name^="barang_id"]').forEach(el => el.remove());
                form.querySelectorAll('input[name^="jumlah"]').forEach(el => el.remove());
                form.querySelectorAll('input[name^="harga_satuan"]').forEach(el => el.remove());

                // Tambahkan input hidden dari items
